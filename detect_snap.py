...
class FrameConsumer:
    def __init__(self, frame_queue: queue.Queue):
        ...
        self.detector = apriltag.Detector(
            apriltag.DetectorOptions(
                ...
                nthreads=4,
                quad_decimate=0.5,   # Lowering the decimate for higher resolution
    ...
    def detect_and_draw_apriltags(self, gray_image, cam_id):
        try:
            # Save frames to inspect later
            cv2.imwrite(f'gray_image_before_blur_{cam_id}.png', gray_image)

            # Apply Gaussian blur to reduce noise
            gray_image_blurred = cv2.GaussianBlur(gray_image, (5, 5), 0)
            cv2.imwrite(f'gray_image_after_blur_{cam_id}.png', gray_image_blurred)
